...
TensorFromStructsExecutor<KeyBufferType>::execute(uint32_t docId)
{
    ...
    // Create builder based on cell type
    if (_cellType == CellType::FLOAT) {
        auto builder = factory.create_value_builder<float>(_type, 1, 1, size);
        for (size_t i = 0; i < size; ++i) {
            std::string key(_keyBuffer[i].value());
            std::vector<std::string_view> addr = {key};
            auto cell_array = builder->add_subspace(addr);
            cell_array[0] = static_cast<float>(_valueBuffer[i]);
        }
        _tensor = builder->build(std::move(builder));
    } else {
        // Default to double
        auto builder = factory.create_value_builder<double>(_type, 1, 1, size);
        for (size_t i = 0; i < size; ++i) {
            std::string key(_keyBuffer[i].value());
            std::vector<std::string_view> addr = {key};
            auto cell_array = builder->add_subspace(addr);
            cell_array[0] = _valueBuffer[i];
        }
        _tensor = builder->build(std::move(builder));
    }
